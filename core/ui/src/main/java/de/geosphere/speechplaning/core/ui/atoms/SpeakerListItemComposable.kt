...
fun SpeakerListItemComposable(
    speakers: Speaker,
    modifier: Modifier,
    avatarProvider: AvatarProvider = koinInject(),
...
private fun SpeakerListItemPreview() {
    // Manuelles Setup für die Preview
    val mockResourceProvider = object : AvatarResourceProvider {
        override fun getAvatarResource(status: SpiritualStatus): Int {
            // Gib hier die echten Drawable-IDs aus dem :theme Modul zurück,
            // da die Preview darauf zugreifen kann.
            return when (status) {
                SpiritualStatus.ELDER -> de.geosphere.speechplaning.theme.R.drawable.business_man_man_avatar_icon
                SpiritualStatus.MINISTERIAL_SERVANT -> de.geosphere.speechplaning.theme.R.drawable.man_avatar_male_icon
                else -> de.geosphere.speechplaning.theme.R.drawable.man_goatee_user_avatar_icon
            }
        }
    }
    val previewAvatarProvider = AvatarProvider(mockResourceProvider)
    ...
    SpeechPlaningTheme {
        Column {
            SpeakerListItemComposable(
                ...
                modifier = Modifier,
                avatarProvider = previewAvatarProvider,
